...
1.  **Refactor Test Examples**:
    ...
    *   Refactor examples to focus directly on the specific module being tested, utilizing Terratest orchestration for dependent modules where appropriate.
    *   Ensure there is at least one overarching End-to-End (E2E) example representing an "all-in-one" apply step.
...
1.  **Directory Restructuring (`./test/tests/` to `./test/`)**:
    ...
    *   Create a `./test/config` package dedicated to setting up the testing fixture and variables.
    *   Isolate individual tests into their own packages, importing both the `test` and `config` packages.
...
2.  **Dependency Updates**:
    ...
    *   Update `github.com/aws/aws-sdk-go` to `aws-sdk-go-v2`.
