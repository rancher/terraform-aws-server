...
module "this" {
  ...
  server_user = {
    user                     = local.username
    aws_keypair_use_strategy = "skip" # add the keypair to the server directly using cloud-init
    ssh_key_name             = ""     # this attribute is not necessary, but required
